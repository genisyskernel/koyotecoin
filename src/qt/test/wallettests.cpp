...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<KoyotecoinAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = KoyotecoinUnits::formatWithUnit(unit, expected_balance, false, KoyotecoinUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_koyotecoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_koyotecoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_koyotecoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
    ...
    // Amount input
    KoyotecoinAmountField* amountInput = receiveCoinsDialog.findChild<KoyotecoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("koyotecoin:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_koyotecoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
