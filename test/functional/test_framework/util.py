...
def wait_until_helper(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `KoyotecoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "koyotecoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "koyotecoin.conf")):
        with open(os.path.join(datadir, "koyotecoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
