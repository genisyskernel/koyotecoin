...
def main():
    ...
    # Build list of tests
    test_list_all = parse_test_list(fuzz_bin=os.path.join(
    ...
    logging.debug("{} fuzz target(s) found: {}".format(
    ...
    logging.info("{} of {} detected fuzz target(s) selected: {}".format(
    ...
    if not args.generate:
        ...
        if test_list_missing_corpus:
            ...
            logging.info(
                "Please consider adding a fuzz corpus at https://github.com/koyotecoin/qa-assets")
    ...
    try:
        help_output = subprocess.run(
            ...
            env=get_fuzz_env(
                target=test_list_selection[0], source_dir=config['environment']['SRCDIR']),
            timeout=20,
            check=True,
...
def merge_inputs(*, fuzz_pool, corpus, test_list, src_dir, build_dir, merge_dir):
    logging.info(
        "Merge the inputs from the passed dir into the corpus_dir. Passed dir {}".format(merge_dir))
    jobs = []
    for t in test_list:
        args = [
            os.path.join(build_dir, 'src', 'test', 'fuzz', 'fuzz'),
            ...
            '-shuffle=0',
            '-prefer_small=1',
            # Also done by oss-fuzz https://github.com/google/oss-fuzz/issues/1406#issuecomment-387790487
            '-use_value_profile=1',
...
def run_once(*, fuzz_pool, corpus, test_list, src_dir, build_dir, use_valgrind):
    ...
    for future in as_completed(jobs):
        ...
        try:
            ...
            logging.info("Target \"{}\" failed with exit code {}".format(
