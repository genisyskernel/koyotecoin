...
- Update the versions.
- Create the draft, named "*version* Release Notes Draft", as a [collaborative wiki](https://github.com/koyotecoin-core/koyotecoin-devwiki/wiki/_new).
...
- Merge the release notes from [the wiki](https://github.com/koyotecoin-core/koyotecoin-devwiki/wiki/) into the branch.
- Ensure the "Needs release note" label is removed from all relevant pull requests and issues.

#### Tagging a release (candidate)

To tag the version (or release candidate) in git, use the `make-tag.py` script from [koyotecoin-maintainer-tools](https://github.com/koyotecoin-core/koyotecoin-maintainer-tools). From the root of the repository run:
...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/koyotecoin-core/guix.sigs.git
    git clone https://github.com/koyotecoin-core/koyotecoin-detached-sigs.git
    git clone https://github.com/koyotecoin/koyotecoin.git

### Write the release notes

Open a draft of the release notes for collaborative editing at https://github.com/koyotecoin-core/koyotecoin-devwiki/wiki.

For the period during which the notes are being edited on the wiki, the version on the branch should be wiped and replaced with a link to the wiki which should be used for all announcements until `-final`.

Generate the change log. As this is a huge amount of work to do manually, there is the `list-pulls` script to do a pre-sorting step based on github PR metadata. See the [documentation in the README.md](https://github.com/koyotecoin-core/koyotecoin-maintainer-tools/blob/master/README.md#list-pulls).
...
- Detached signatures will then be committed to the [koyotecoin-detached-sigs](https://github.com/koyotecoin-core/koyotecoin-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
- Upload to the koyotecoin.org server (`/var/www/bin/koyotecoin-core-${VERSION}/`):
    1. The contents of each `./koyotecoin/guix-build-${VERSION}/output/${HOST}/` directory, except for
       ...
       as save storage space *do not upload these to the koyotecoin.org server,
       nor put them in the torrent*.

       ```sh
       find guix-build-${VERSION}/output/ -maxdepth 2 -type f -not -name "SHA256SUMS.part" -and -not -name "*debug*" -exec scp {} user@koyotecoin.org:/var/www/bin/koyotecoin-core-${VERSION} \;
       ```
...
- Create a torrent of the `/var/www/bin/koyotecoin-core-${VERSION}` directory such
  that at the top level there is only one file: the `koyotecoin-core-${VERSION}`
...
- Update other repositories and websites for new version
  ...
  - koyotecoin.org maintained versions update:
    [table](https://github.com/koyotecoin-core/koyotecoin.org/commits/master/_includes/posts/maintenance-table.md)
  ...
  - koyotecoin.org RPC documentation update
      ...
      - Clone the [koyotecoin.org repository](https://github.com/koyotecoin-core/koyotecoin.org)
  ...
  - Update packaging repo
      ...
      - Push the snap, see https://github.com/koyotecoin-core/packaging/blob/master/snap/build.md
...
- Announce the release:
  ...
  - koyotecoin-dev and koyotecoin-core-dev mailing list

  - Koyotecoin Core announcements list https://koyotecoin.org/list/announcements/join/

  - Koyotecoin Core Twitter https://twitter.com/koyotecoinorg
