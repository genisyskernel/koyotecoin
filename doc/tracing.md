# User-space, Statically Defined Tracing (USDT) for Koyotecoin Core

Koyotecoin Core includes statically defined tracepoints to allow for more
...
**Note**: Koyotecoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Koyotecoin Core
...
Multiple tools can list the available tracepoints in a `koyotecoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Koyotecoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/koyotecoind
stap net        outbound_message 0x0000000000107c05 /src/koyotecoind
stap validation block_connected  0x00000000002fb10c /src/koyotecoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Koyotecoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/koyotecoind | grep NT_STAPSDT -A 4 -B 2
