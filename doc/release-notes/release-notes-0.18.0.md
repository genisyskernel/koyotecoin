...
`contrib/init/koyotecoind.service`, it has been changed to use
`/var/lib/koyotecoind` as the data directory instead of
`~koyotecoin/.koyotecoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/koyotecoind` will
exist has enough space (check using `df -h /var/lib/koyotecoind`), and
...
`~koyotecoin/.koyotecoin`. This change makes Koyotecoin Core more consistent with
...
`datadir` option in `/etc/koyotecoin/koyotecoin.conf` will have no effect.
...
- Further information is added to the [JSON-RPC
  documentation](https://github.com/koyotecoin/koyotecoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/koyotecoin/koyotecoin/blob/master/doc/koyotecoin-conf.md)
  about the `koyotecoin.conf` file describes how to use it to configure
  Koyotecoin Core.

- A new document introduces Koyotecoin Core's BIP174 [Partially-Signed
...
- The [output script
  descriptor](https://github.com/koyotecoin/koyotecoin/blob/master/doc/descriptors.md)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/koyotecoin/koyotecoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Koyotecoin Core knows enough about the address's scriptPubKey,
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/koyotecoin/koyotecoin/blob/master/doc/descriptors.md).
...
- Since version 0.16.0, Koyotecoin Core’s built-in wallet has defaulted to
  ...
  to bech32 addresses, and if the Koyotecoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Koyotecoin Core 0.19 (approximately November 2019).
...
- #12490 Remove deprecated wallet rpc features from `koyotecoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libkoyotecoin_server.a` (jnewbery)
...
- #13926 [Tools] koyotecoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like koyotecoind (MarcoFalke)
...
- #14521 qt, docs: Fix `koyotecoin-qt -version` output formatting (hebasto)
...
- #11625 Add KoyotecoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libkoyotecoin_server` (laanwj)
...
- #15154 Configure: koyotecoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/koyotecoin-wallet (Sjors)
...
- #12775 Integration of property based testing into Koyotecoin Core (Christewart)
...
- #12246 Bugfix: Only run koyotecoin-tx tests when koyotecoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_koyotecoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_koyotecoin` (MarcoFalke)
...
- #14186 koyotecoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_koyotecoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update koyotecoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/koyotecoin-conf.md (hebasto)
...
- #15354 Add missing `koyotecoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct koyotecoinconsensus_version in shared-libraries.md (fanquake)
