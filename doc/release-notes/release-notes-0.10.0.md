...
optimized for the curve Koyotecoin uses which was created by Koyotecoin
...
the curve Koyotecoin uses and we have reason to believe that
...
critical to Koyotecoin's consensus available to other applications, e.g. to language
bindings such as [python-koyotecoinlib](https://pypi.python.org/pypi/python-koyotecoinlib) or
alternative node implementations.

This library is called `libkoyotecoinconsensus.so` (or, `.dll` for Windows).
Its interface is defined in the C header [koyotecoinconsensus.h](https://github.com/koyotecoin/koyotecoin/blob/0.10/src/script/koyotecoinconsensus.h).
...
While the Koyotecoin protocol has always supported these types of script,
...
koyotecoin-tx is a newly introduced command line utility designed to enable easy
manipulation of koyotecoin transactions. A summary of its operation may be
obtained via "koyotecoin-tx --help" Transactions may be created or signed in a
manner similar to the RPC raw tx API. Transactions may be updated, deleting
inputs or outputs, or appending new inputs and outputs. Custom scripts may be
easily composed using a simple text notation, borrowed from the koyotecoin test
suite.
...
If you are solo mining, this will affect you the moment you upgrade Koyotecoin
...
Koyotecoin Core now supports BIP 22 long polling, so mining software can be
notified immediately of new templates rather than having to poll periodically.

Support for BIP 23 block proposals is now available in Koyotecoin Core's
...
non-standard since Koyotecoin v0.8.0 (released in February 2013), but were
...
- `b750cf1` Remove cli functionality from koyotecoind
...
- `cbe39a3` Add "koyotecoin-tx" command line utility and supporting modules
...
- `58fda4d` Update seed IPs, based on koyotecoin.sipa.be crawler data
...
- `c101c76` build: Add --with-utils (koyotecoin-cli and koyotecoin-tx, default=yes). Help string consistency tweaks. Target sanity check fix
...
- `bd0aa10` Replace the temporary file hack currently used to change Koyotecoin-Qt's dock icon (OS X) with a buffer-based solution
...
- `d789386` Add "it works" test for koyotecoin-tx
- `df4d61e` Add koyotecoin-tx tests
...
- `7a41614` tests: allow rpc-tests to get filenames for koyotecoind and koyotecoin-cli from the environment
...
- `0c3e101` contrib: Added systemd .service file in order to help distributions integrate koyotecoind
