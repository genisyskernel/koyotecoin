...
<https://koyotecoin.org/bin/koyotecoin-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/koyotecoin/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Koyotecoin Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Koyotecoin Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/koyotecoin/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/koyotecoin/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/koyotecoin/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/koyotecoin/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/koyotecoin/bips/blob/master/bip-0342.mediawiki)),
...
- A new `koyotecoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `koyotecoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `koyotecoin-cli -getinfo` is now expanded to return a JSON
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `koyotecoin-wallet` tool which performs the salvage
...
Koyotecoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`koyotecoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Koyotecoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `koyotecoin-cli createwallet` or `koyotecoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/koyotecoin/koyotecoin/issues).
...
Users who already have a Koyotecoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Koyotecoin Core.
...
The change to using descriptors changes the default derivation paths used by Koyotecoin Core
...
- A new `fee_rate` parameter/option denominated in howloshis per vbyte (howl/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in KYC per 1,000
  vbytes (KYC/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from KYC/kB
  to howl/vB and the help documentation in KYC/kB is updated to KYC/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from KYC/kvB to howl/vB and the
  ...
  difference between KYC/kvB and howl/vB units means that a transaction with a
  fee rate mistakenly calculated in KYC/kvB rather than howl/vB should raise an
  error due to the fee rate being set too low. In the worst case, the
  transaction may send at 1 howl/vB, but as Replace-by-Fee (BIP125 RBF) is active
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  ...
  `\<datadir\>/settings.json` and augments any command line or `koyotecoin.conf`
...
- #19284 Add seed.koyotecoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in koyotecoin-wallet tool (jonasschnelli)
...
- #20305 Introduce `fee_rate` howl/vB param/option (jonatack)
...
- #19133 rpc, cli, test: add koyotecoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in koyotecoin-cli (laanwj)
...
- #17929 add linker optimihowlion flags to gitian & guix (Linux) (fanquake)
...
- #18297 Use pkg-config in KOYOTECOIN_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for koyotecoin-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Koyotecoin Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrekoyotecoin)
...
- #18817 Document differences in koyotecoind and koyotecoin-qt locale handling (practicalswift)
