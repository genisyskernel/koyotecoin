...
prefixing the option with the network, such as:
    ...
    main.uacomment=koyotecoin
    test.uacomment=koyotecoin-testnet
...
be used in V0.17 by starting koyotecoind with the '-deprecatedrpc=accounts'
...
- When koyotecoin is not started with any `-wallet=<path>` options, the name of
  ...
  now the empty string `""` instead of `"wallet.dat"`. If koyotecoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart koyotecoind with
...
- When running koyotecoind with `-debug` but without `-daemon`, logging to stdout
...
- #11962 `0a01843` add seed.koyotecoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in KYC (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI koyotecoin:// (krab)
...
- #13264 `2a7c53b` Howloshi unit (GreatSock)
...
- #13528 `b877c39` Move KoyotecoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libkoyotecoin-util to libkoyotecoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for koyotecoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling koyotecoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use KoyotecoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to koyotecoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use KoyotecoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use KoyotecoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use KoyotecoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for KoyotecoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_koyotecoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.koyotecoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_koyotecoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in koyotecoind systemd service file (Flowdalic)
- #12652 `55f490a` koyotecoin-cli: Provide a better error message when koyotecoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to koyotecoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for koyotecoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` koyotecoin-tx: Stricter check for valid integers (domob1812)
...
- #12877 `2b54155` Use koyotecoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update koyotecoin-dot-org links in release-process.md (fanquake)
...
- #13614 `17e9106` Update command line help for -printtoconsole and -debuglogfile (howlwo, fanquake)
...
- #13625 `7146672` Add release notes for -printtoconsole and -debuglogfile changes (howlwo)
...
- #13742 `d2186b3` Adjust koyotecoin.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed koyotecoin-cli -help output for help2man (hebasto)
