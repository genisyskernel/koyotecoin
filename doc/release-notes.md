...
Some Koyotecoin services today expect that the first version of an
...
default for years, and several contributors to Koyotecoin Core are
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the Koyotecoin Core wallet yet.
  You can find more about Miniscript on the [reference website](https://koyotecoin.sipa.be/miniscript/). (#24148)

- The `tr()` output descriptor now supports multisig scripts through the `multi_a()` and
  `sortedmulti_a()` functions. (#24043)

- To help prevent fingerprinting transactions created by the Koyotecoin Core wallet, change output
