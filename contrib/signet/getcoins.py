...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing koyotecoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='koyotecoin-cli', help='koyotecoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Koyotecoin address to which the faucet should send')
...
parser.add_argument('koyotecoin_cli_args', nargs='*', help='Arguments to pass on to koyotecoin-cli (default: -signet)')

args = parser.parse_args()

if args.koyotecoin_cli_args == []:
    args.koyotecoin_cli_args = ['-signet']


def koyotecoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.koyotecoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = koyotecoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = koyotecoin_cli(['getnewaddress', 'faucet', 'bech32'])
